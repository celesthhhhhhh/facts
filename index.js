...
(() => {
  ...
  const CATEGORIES = Object.freeze({
    characters: { label: 'Characters & Relationships', icon: '👤', short: 'CHARACTERS' },
    events:     { label: 'Events & Consequences',  icon: '📅', short: 'EVENTS'   },
    secrets:    { label: 'Secrets & Hidden Things',       icon: '🔒', short: 'SECRETS'   },
    flashbacks: { label: 'Flashbacks & Past',      icon: '🌀', short: 'FLASHBACKS'  },
  });

  const IMPORTANCE = Object.freeze({
    high:   { label: '🔴 High', color: '#e55' },
    medium: { label: '🟡 Medium', color: '#ca3' },
  ...
  const DEFAULT_PROMPT_TEMPLATE =
    `[FACTS MEMORY]\nKey facts about the world, characters and events of this RP:\n{{facts}}\n[/FACTS MEMORY]`;
  ...
  async function getChatState(createIfMissing = false) {
    ...
    if (recovered) {
      ...
      if (!chatMetadata[exact].scanLog) chatMetadata[exact].scanLog = [];
      console.info(`[FMT] Facts recovered from key ${recovered} → ${exact}`);
      setTimeout(() => toastr.success(
        `🧠 FMT: facts recovered (${chatMetadata[exact].facts.length} pcs.)`,
        'Data Recovery',
        { timeOut: 5000 }
      ), 500);
      await saveMetadata();
  ...
  async function fetchModels() {
    ...
    if (!base) throw new Error('Specify an Endpoint');
    ...
    throw new Error('Model list unavailable. Enter the model manually.');
  }

  async function testApiConnection() {
    const s    = getSettings();
    const base = getBaseUrl();
    if (!base) throw new Error('Endpoint not set');
    ...
    throw new Error('None of the endpoints responded correctly');
  }
  ...
  async function aiGenerate(userPrompt, systemPrompt) {
    ...
    if (base) {
      ...
      if (s.fallbackEnabled === false) {
        throw new Error(`Custom API did not respond: ${errSummary}`);
      }
      console.warn(`[FMT] Custom API did not respond (${errSummary}) — falling back to ST`);
      toastr.warning('[FMT] Custom API unavailable — using ST', '', { timeOut: 3000 });
    }
    ...
    if (typeof c.generateRaw !== 'function') {
      throw new Error(
        'generateRaw is not available in this version of ST. ' +
        'Make sure ST is updated, or set up a custom API in the 🔌 API section of FMT settings.'
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Generation error via ST: ${e.message}. ` +
        'Check: 1) whether a model is connected in Chat Completion, 2) whether an active RP chat is interfering.'
      );
    }
    ...
    if (!result?.trim()) {
      throw new Error(
        ...
        '1) no model is connected in ST, ' +
        '2) the context is too large — reduce the "Depth" in the scan settings, ' +
        '3) the model cannot return clean JSON — connect a custom API with GPT-4o or similar.'
      );
    }
  ...
  function buildSystemPrompt(existingFacts) {
    const existing = existingFacts.length
      ...
      : '';
    return `You are an RP dialogue analyst. Extract important facts from the conversation.

WHAT COUNTS AS A FACT: character names/roles/traits, relationships, events with consequences, hidden motives, secrets, incriminating information, decisions.
WHAT DOES NOT COUNT: atmosphere with no plot significance, actions without consequences, general emotions.

CATEGORIES:
- characters — characters, relationships, appearance, personality traits, backstory
- events     — events that happened, decisions, consequences
- secrets    — secrets, hidden motives, incriminating information
- flashbacks — a character's memories of the past: when {{char}} recalls something, sees images, or mentions long-past events or trauma. This is a SEPARATE category — don't confuse it with events.

IMPORTANCE: high (key) | medium (useful context) | low (minor)
Fact text: up to 15 words, third person.
Return ONLY a valid JSON array with no preamble or markdown:
[{"category":"characters|events|secrets|flashbacks","text":"fact","importance":"high|medium|low"}]
If there are no new facts — return [].${existing}`;
  }
  ...
  async function extractFacts(fromIdx, toIdx) {
    ...
    const user     = `${charCard ? `CHARACTER CARD:\n${charCard}\n\n` : ''}━━━ MESSAGES ━━━\n${text}\n\nExtract new facts. Return a JSON array.`;
    ...
    if (!parsed) {
      ...
      console.warn(`[FMT] Model returned non-JSON: "${preview}"`);
      throw new Error(
        `The model returned a response that is not JSON. First characters: "${raw.slice(0, 80)}". ` +
        'Try another model or a custom API with a smarter model (GPT-4o, Claude, etc.)'
      );
    }
  ...
  async function detectFactMarkers(messageText) {
    ...
    for (const m of matches) {
      ...
      pool.push(text);
      changed = true;
      toastr.info(`🧠 New fact: "${text}"`, 'FMT Auto-marker', { timeOut: 4000 });
    }
  ...
  async function detectFlashbackMarkers(messageText) {
    ...
    for (const m of matches) {
      ...
      pool.push(text);
      changed = true;
      toastr.info(`🌀 Flashback: "${text}"`, 'FMT Flashback', { timeOut: 5000 });
    }
  ...
  async function runScan(mode = 'manual') {
    if (scanInProgress) { toastr.warning('[FMT] Scan already in progress…'); return; }
    ...
    if (!Array.isArray(chat) || !chat.length) {
      toastr.warning('[FMT] Chat history is empty or not loaded yet. Try again in a second.');
      return;
    }
    ...
    try {
      ...
      if (fromIdx >= toIdx) {
        if (mode === 'manual') toastr.info('No new messages to analyze', 'FMT');
        return;
      }
      ...
      if (mode === 'manual') {
        if (added === 0) toastr.info('🔍 No new facts found', 'FMT', { timeOut: 4000 });
        else toastr.success(`✅ Extracted: <b>${added}</b> facts`, 'FMT', { timeOut: 5000, escapeHtml: false });
      }
    } catch (e) {
      console.error('[FMT] scan failed', e);
      toastr.error(`[FMT] Error: ${e.message}`);
    } finally {
  ...
  async function runScanRange() {
    ...
    const chat = c.chat ?? c.getChat?.() ?? [];
    if (!Array.isArray(chat) || !chat.length) {
      toastr.warning('[FMT] Chat history is empty or not loaded yet.');
      return;
    }

    const total = chat.length;

    c.Popup.show.text('🎯 FMT — Scan Range',
      `<div style="font-size:13px;color:var(--SmartThemeBodyColor,#c8deff)">
        <div style="margin-bottom:12px;opacity:.75">
          Total messages in chat: <b style="color:var(--SmartThemeQuoteColor,#90b8f8)">${total}</b><br>
          <span style="font-size:11px;opacity:.7">Numbering starts at 1. Hidden messages and summaries are skipped automatically.</span>
        </div>
        <div style="display:flex;gap:10px;align-items:center;margin-bottom:14px;flex-wrap:wrap">
          <div style="display:flex;flex-direction:column;gap:4px">
            <label style="font-size:11px;opacity:.65;text-transform:uppercase;letter-spacing:.05em">From #</label>
            <input id="fmt_range_from" type="number" min="1" max="${total}" value="1"
              style="width:90px;padding:7px 10px;border-radius:20px;border:1px solid var(--SmartThemeBorderColor,rgba(100,160,255,0.3));background:var(--SmartThemeBlurTintColor,rgba(5,12,25,.9));color:var(--SmartThemeBodyColor,#d8e8ff);font-size:14px;font-weight:700;text-align:center">
          </div>
          <div style="font-size:20px;opacity:.4;padding-top:18px">→</div>
          <div style="display:flex;flex-direction:column;gap:4px">
            <label style="font-size:11px;opacity:.65;text-transform:uppercase;letter-spacing:.05em">To #</label>
            <input id="fmt_range_to" type="number" min="1" max="${total}" value="${total}"
              style="width:90px;padding:7px 10px;border-radius:20px;border:1px solid var(--SmartThemeBorderColor,rgba(100,160,255,0.3));background:var(--SmartThemeBlurTintColor,rgba(5,12,25,.9));color:var(--SmartThemeBodyColor,#d8e8ff);font-size:14px;font-weight:700;text-align:center">
          </div>
          <div style="display:flex;flex-direction:column;gap:4px">
            <label style="font-size:11px;opacity:.65;text-transform:uppercase;letter-spacing:.05em">Total</label>
    ...
    setTimeout(() => {
      ...
      document.getElementById('fmt_range_go')?.addEventListener('click', async () => {
        ...
        if (fromNum > toNum) {
          $status.css('color', '#ff7070').text('❌ "From" cannot be greater than "To"');
          return;
        }
        if (fromNum < 1 || toNum > total) {
          $status.css('color', '#ff7070').text(`❌ Valid range: 1 – ${total}`);
          return;
        }
        ...
        if (scanInProgress) { $status.css('color', '#ca3').text('⏳ Scan already in progress…'); return; }

        const $goBtn = $('#fmt_range_go');
        $goBtn.prop('disabled', true).text('⏳ Analyzing…');
        $status.css('color', 'rgba(180,200,240,.6)').text(`Scanning messages ${fromNum}–${toNum}…`);
        scanInProgress = true;
        ...
        try {
          ...
          if (added === 0) {
            $status.css('color', '#888').text('No new facts found in this range');
          } else {
            $status.css('color', '#5cd8a4').text(`✅ Extracted: ${added} facts`);
            toastr.success(`✅ Range ${fromNum}–${toNum}: <b>${added}</b> facts`, 'FMT', { timeOut: 5000, escapeHtml: false });
          }
        } catch (e) {
          ...
          scanInProgress = false;
          $goBtn.prop('disabled', false).text('🔍 Scan range');
        }
      });
    }, 0);
  }
  ...
  function ensureFab() {
    ...
    $('body').append(`
      <div id="fmt_fab">
        <button type="button" id="fmt_fab_btn" title="Open facts tracker">
          <div>🧠</div>
          <span id="fmt_fab_count">0</span>
          <div class="fmt-mini"></div>
        </button>
        <button type="button" id="fmt_fab_hide" title="Hide widget">✕</button>
      </div>
    `);
    ...
    $('#fmt_fab_hide').on('click', async () => {
      ...
      await renderWidget();
      toastr.info('Widget hidden (re-enable it in the extension settings)');
    });
    initFabDrag();
    applyFabPosition();
    applyFabScale();
  ...
  function ensureDrawer() {
    ...
    $('body').append(`
      <aside id="fmt_drawer" aria-hidden="true">
        <header>
          <div class="topline">
            <div class="title">🧠 Facts Memory</div>
        ...
        <div class="fmt-toolbar">
          <input type="text" id="fmt_search" placeholder="Search facts…" autocomplete="off">
        ...
        <div class="fmt-filters">
          <button class="fmt-filter-btn active" data-cat="all">All</button>
        ...
        <div class="footer">
          <button type="button" id="fmt_scan_btn">🔍 Scan</button>
          <button type="button" id="fmt_scan_range_btn" title="Select range">🎯 Range</button>
          <button type="button" id="fmt_flashback_btn" title="Random flashback">⚡ Flashback</button>
          <button type="button" id="fmt_export_btn" title="Export">📤</button>
          <button type="button" id="fmt_import_btn" title="Import">📥</button>
          <button type="button" id="fmt_show_prompt_btn" title="Prompt">📝</button>
          <button type="button" id="fmt_scanlog_btn" title="Scan log">📋</button>
          <button type="button" id="fmt_recover_btn" title="Recover facts">🔄</button>
          <button type="button" id="fmt_clear_btn" title="Clear all">🗑️</button>
          <button type="button" id="fmt_close2" style="pointer-events:auto">Close</button>
        </div>
      </aside>
    `);
  ...
  function renderFactRow(fact) {
    ...
    const ts      = new Date(fact.ts || 0).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit' });
    ...
    return `
      <div class="fmt-fact-row${dis ? ' fmt-fact-disabled' : ''}"
        ...
        <div class="fmt-card-top">
          <select class="fmt-inline-cat" data-id="${fact.id}" title="Category">${catOpts}</select>
          <span class="fmt-imp-dot" style="background:${impMeta.color}" title="${escHtml(impMeta.label)}"></span>
          <select class="fmt-inline-imp" data-id="${fact.id}" title="Importance">${impOpts}</select>
        </div>
        <span class="fmt-fact-text" data-id="${fact.id}" title="Click to edit">${escHtml(fact.text)}</span>
        <div class="fmt-card-bottom">
          <span class="fmt-fact-date">${ts}</span>
          <button class="fmt-flash-btn" data-id="${fact.id}" title="Flashback">⚡</button>
          <button class="fmt-toggle-btn" data-id="${fact.id}" title="${dis ? 'Enable' : 'Disable'}">${dis ? '▶' : '⏸'}</button>
          <button class="fmt-delete-btn" data-id="${fact.id}" title="Delete">✕</button>
        </div>
      </div>`;
  }
  ...
  async function renderDrawer() {
    ...
    $('#fmt_subtitle').text(`${charName} · ${total} facts · ${active} active`);
    ...
    $('#fmt_token_bar').html(
      block
        ? `<span class="fmt-tok-label">Injection: ~<b>${tokens}</b> tokens · ${active}/${maxF} facts</span>`
        : `<span class="fmt-tok-label fmt-tok-empty">Injection is empty — no active facts above the threshold</span>`
    );
    ...
    let html = `
      <div class="fmt-add-block">
        <input type="text" id="fmt_add_text" placeholder="Add a fact manually…" maxlength="120">
        ...
        <button id="fmt_add_btn">+ Add</button>
      </div>`;

    if (total === 0) {
      html += `<div class="fmt-empty">No facts yet.<br>Click <b>🔍 Scan</b> — AI will extract what's important from the chat history.</div>`;
    } else {
        ...
        html += `
          <div class="fmt-cat-section" data-cat="${cat}">
            <div class="fmt-cat-header" data-collapse-cat="${cat}">
              ...
              <span class="fmt-cat-count">${items.length}${disabledN ? ` <span class="fmt-cat-dis">${disabledN} disabled</span>` : ''}</span>
            </div>
  ...
  async function addFactManual() {
    ...
    if (!text) { toastr.warning('Enter fact text'); return; }
    const state = await getChatState(true);
  ...
  async function toggleDisableFact(id) {
    ...
    if (row) {
      ...
      if (btn) { btn.textContent = fact.disabled ? '▶' : '⏸'; btn.title = fact.disabled ? 'Enable' : 'Disable'; }
    }
    await ctx().saveMetadata();
    await updateInjectedPrompt();
    await renderWidget();
    ...
    $('#fmt_token_bar').html(
      block
        ? `<span class="fmt-tok-label">Injection: ~<b>${tokens}</b> tokens · ${active}/${maxF} facts</span>`
        : `<span class="fmt-tok-label fmt-tok-empty">Injection is empty</span>`
    );
  }
  ...
  async function recoverFacts() {
    ...
    if (!candidates.length) {
      toastr.warning('[FMT] No saved facts found for this character under any key');
      return;
    }
    ...
    if (bestKey === exact) {
      toastr.info(`[FMT] Data is already up to date (${n} facts)`);
      return;
    }
    ...
    await renderDrawer();
    await renderWidget();
    toastr.success(`✅ Recovered ${n} facts!`, 'FMT', { timeOut: 5000 });
  }

  async function clearAllFacts() {
    const { Popup } = ctx();
    const ok = await Popup.show.confirm('Clear all facts?', 'This action cannot be undone.');
    if (!ok) return;
    const state = await getChatState(true);
    ...
    await renderDrawer();
    await renderWidget();
    toastr.success('All facts deleted');
  }
  ...
  async function exportJson() {
    ...
    ctx().Popup.show.text('📤 FMT — Export Facts',
      `<div style="font-family:inherit;font-size:13px">
        <div style="margin-bottom:10px;opacity:.8">
          Character: <b>${escHtml(charName)}</b> · Facts: <b>${total}</b>
        </div>
    ...
    setTimeout(() => {
      ...
      document.getElementById('fmt_export_copy')?.addEventListener('click', () => {
        navigator.clipboard?.writeText(json).then(
          ...
          () => toastr.error('Failed to copy — select the text manually')
        );
      });
    }, 0);
  }
  ...
  async function importJson() {
    ctx().Popup.show.text('📥 FMT — Import Facts',
      `<div style="font-family:inherit;font-size:13px">
        <div style="margin-bottom:10px;font-weight:700;opacity:.9">Load from a file or paste JSON:</div>
    ...
    setTimeout(() => {
      ...
      document.getElementById('fmt_import_file_input')?.addEventListener('change', (ev) => {
        ...
        reader.onerror = () => toastr.error('Failed to read file');
      ...
      document.getElementById('fmt_import_apply')?.addEventListener('click', async () => {
        ...
        if (!raw) { toastr.warning('Paste JSON or choose a file'); return; }
        try {
          ...
          await renderDrawer();
          await renderWidget();
          toastr.success(`✅ Imported ${p.facts.length} facts`);
          const st = document.getElementById('fmt_import_status');
          if (st) st.textContent = `✅ Done (${p.facts.length} facts)`;
        } catch (e) {
          toastr.error('[FMT] Invalid JSON: ' + e.message);
          const st = document.getElementById('fmt_import_status');
  ...
  async function showPromptPreview() {
    const state    = await getChatState();
    const settings = getSettings();
    const block    = buildInjectedBlock(state, settings) || '[No active facts above the threshold]';
    const tokens   = estimateTokens(block);
    await ctx().Popup.show.text(
      `FMT — Prompt (~${tokens} tokens)`,
  ...
  async function showScanLog() {
    ...
    if (!log.length) { toastr.info('Scan log is empty'); return; }
    const rows = log.map(e => {
      const d = new Date(e.ts).toLocaleString('en-GB');
    ...
    await ctx().Popup.show.text('FMT — Scan History', `
      <table style="width:100%;border-collapse:collapse;font-size:12px;color:inherit">
        <thead><tr style="color:var(--SmartThemeQuoteColor,#90b8f8);border-bottom:1px solid rgba(100,160,255,0.2)">
          ...
          <th>Mode</th><th>Messages</th><th>Added</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>`);
  }
  ...
  async function mountSettingsUi() {
    ...
    $(document).off('click.fmt_apimode').on('click.fmt_apimode', '.fmt-api-mode-btn', function () {
      ...
      if (mode === 'st') {
        ...
        _workingApiConfig = null;
        ctx().saveSettingsDebounced();
        toastr.info('[FMT] Using ST (current model)', '', { timeOut: 2500 });
      } else {
    ...
    $('#fmt_test_api').on('click', async () => {
      ...
      $status.css('color', 'var(--fmt-text-dim)').text('Trying endpoints…');
      try {
        const cfg = await testApiConnection();
        _workingApiConfig = { base: getBaseUrl(), url: cfg.url, builder: cfg.builder };
        $status.css('color', 'var(--fmt-green)').text(`✅ Works: ${cfg.url.replace(getBaseUrl(), '')}`);
        toastr.success('[FMT] API responded correctly');
      } catch (e) {
    ...
    $('#fmt_refresh_models').on('click', async () => {
      ...
      try {
        ...
        toastr.warning(`[FMT] ${e.message} — enter model manually`);
      } finally {
    ...
    $(document)
      ...
      .on('click.fmt_settings', '#fmt_reset_pos_btn', () => {
        ...
        setFabDefault(); toastr.success('Position reset');
      });
  }
  ...
  function buildFlashBlock(fact) {
    const catMeta = CATEGORIES[fact.category] || CATEGORIES.events;
    return `[FLASHBACK — FOR THIS RESPONSE ONLY]
In this response, {{char}} suddenly — in the middle of a scene or conversation — experiences a brief involuntary memory or inner image connected to the following fact:

${catMeta.icon} "${fact.text}"

Instructions:
- The memory should break through naturally, like a flash — an image, a smell, a sound, a fragment of a phrase
- Don't explain this to the player directly — show it through {{char}}'s behavior, a pause, a change in tone
- The flashback is short (1–3 sentences) and should not take over the whole response
- Afterwards, {{char}} returns to the current scene
[/FLASHBACK]`;
  }
  ...
  async function triggerFlashback(factId = null) {
    ...
    if (!pool.length) {
      toastr.warning('[FMT] No suitable facts for a flashback.');
      return;
    }
    ...
    toastr.info(
      `⚡ Flashback added${qLen > 1 ? ` (queue: ${qLen})` : ''}: "${fact.text.slice(0, 55)}${fact.text.length > 55 ? '…' : ''}"`,
      'FMT',
      { timeOut: 4000 }
    );
  ...
  function removeFromFlashQueue(id) {
    ...
    applyFlashQueue();
    renderFlashQueueUI();
    toastr.info('[FMT] Flashback removed from queue', '', { timeOut: 2000 });
  }
  ...
  function clearFlashQueue() {
    ...
    try { ctx().setExtensionPrompt(FLASHBACK_TAG, '', EXT_PROMPT_TYPES.IN_PROMPT, 0, true); } catch {}
    renderFlashQueueUI();
    toastr.info('[FMT] Flashback queue cleared', '', { timeOut: 2000 });
  }
  ...
  function renderFlashQueueUI() {
    ...
    if (hasQueue) {
      html += `<div class="fmt-fq-section">
        <div class="fmt-fq-title">
          ⏳ Queue (${flashQueue.length})
          <button class="fmt-fq-clear-all" title="Clear entire queue">✕ all</button>
        </div>`;
      flashQueue.forEach((e, i) => {
        ...
        html += `<div class="fmt-fq-row">
          ...
          <button class="fmt-fq-remove" data-qid="${e.id}" title="Remove from queue">✕</button>
        </div>`;
      });
      html += '</div>';
    }
    ...
    if (hasHistory) {
      html += `<div class="fmt-fq-section fmt-fq-history">
        <div class="fmt-fq-title">🕓 Fired (last ${flashHistory.length})</div>`;
      flashHistory.forEach(e => {
        const cat  = CATEGORIES[e.factCat] || CATEGORIES.events;
        const time = new Date(e.fired).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
